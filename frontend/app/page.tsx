      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        {messages.map((msg, i) => (
          <div key={i} className={cn("flex w-full gap-4", msg.role === "user" ? "justify-end" : "justify-start")}>
            {msg.role === "ai" && (
              <div className="flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-md border border-gray-200 bg-white shadow-sm">
                <Bot className="h-5 w-5" />
              </div>
            )}
            <div className={cn(
              "relative flex max-w-[80%] flex-col gap-2 rounded-lg px-4 py-3 text-sm",
              msg.role === "user" ? "bg-gray-900 text-gray-50" : "bg-gray-100 text-gray-900"
            )}>
              <div className="whitespace-pre-wrap">{msg.content}</div>
            </div>
            {msg.role === "user" && (
              <div className="flex h-8 w-8 shrink-0 select-none items-center justify-center rounded-md bg-gray-200">
